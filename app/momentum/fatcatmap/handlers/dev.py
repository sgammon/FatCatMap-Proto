...
class SandboxAddData(FCMRequestHandler):
	...
	def get(self):
		...
		for node in nodes:
			...
			if node.parent() is not None:
				...
				parent = str(None)
			nodes_list.append({'nodeName':node.label, 'group':1, 'nodeKey': {'kind':node.kind(), 'value':str(node.key()), 'id_or_name': node.key().id_or_name(), 'parent':parent}})
		...
		for edge in edges:
			...
			edges_list.append({'source':nodes_index.index(node_alpha), 'target':nodes_index.index(node_beta), 'value': 1})
